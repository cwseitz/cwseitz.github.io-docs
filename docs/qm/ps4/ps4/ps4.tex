...
\end{align*}
where $x_{0} = \sqrt{\frac{\hbar}{m\omega}}$. The probability that $\ket{\alpha}$ is measured to be in the state $\ket{0}$ is given by the inner product

\begin{align*}
\bra{\alpha}\ket{0} &= \pi^{-1/2}x_{0}^{-1}\int_{-\infty}^{\infty} \exp\left(-\frac{1}{2}\left(\frac{x-a}{x_{0}}\right)^{2}\right)\exp\left(-\frac{1}{2}\left(\frac{x}{x_{0}}\right)^{2}\right)dx\\
&= \pi^{-1/2}x_{0}^{-1}\int_{-\infty}^{\infty} \exp\left(-\frac{1}{2}\frac{(x-a)^{2}+x^{2}}{x_{0}^{2}}\right)dx\\
\end{align*}

The numerator can be alternatively written as

\begin{align*}
(x-a)^{2}+x^{2} &= 2x^{2} - 2ax + a^{2} \\
&= 2(x-a/2)^{2} + a^{2}/2
\end{align*}
...
\begin{align*}
\bra{\alpha}\ket{0} &= \pi^{-1/2}x_{0}^{-1}e^{-a^{2}/4x_{0}^{2}}\int_{-\infty}^{\infty} \exp\left(-\frac{(x-a/2)^{2}}{x_{0}^{2}}\right)dx\\
&= e^{-a^{2}/4x_{0}^{2}}
\end{align*}

and the probability is then $e^{-a^{2}/2x_{0}^{2}}$
