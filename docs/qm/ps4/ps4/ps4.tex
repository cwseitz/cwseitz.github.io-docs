...
We are free to choose our zero of potential so we can solve an alternative system where $V(x) = \nu_{0} - \delta(x)\nu_{0}$. We then decompose $\psi(x)$ into two regions $\psi_{I}(x), \psi_{II}(x)$ where 
...
We then solve Schrodingers equation in each region. Both regions have constant potential $|\nu_{0}|$

\begin{align*}
\frac{d\psi_{I}^{2}}{dx^{2}} &= \frac{2m(E-\nu_{0})}{\hbar^{2}}\psi_{I}(x)\\
\frac{d\psi_{II}^{2}}{dx^{2}} &= \frac{2m(E-\nu_{0})}{\hbar^{2}}\psi_{II}(x)
\end{align*}

\begin{align*}
\psi_{I}(x) &= A\exp(\kappa x) + B\exp(-\kappa x)\\
\psi_{II}(x) &= C\exp(\kappa x) + D\exp(-\kappa x)
\end{align*}

Taking limits shows that $B = 0$ and $C=0$. From the symmetry of the potential we should have that $\psi'_{I} = -\psi'_{II}$ which is satisfied when $A=D$. This reduces the solution to

\begin{align*}
\psi_{I}(x) &= A\exp(\kappa x)\\
\psi_{II}(x) &= A\exp(-\kappa x)
\end{align*}

Looking back at Schrodinger's equation, we can see that when $E > \nu_{0}$ the solutions are complex exponentials and are unbound but when $E < \nu_{0}$, the solutions are real exponentials and bound. There are an infinite number of bound states since $E$ and therefore $\kappa$ are continuous parameters. When $E$ hits $\nu_{0}$, we have the solutions for a free particle.
