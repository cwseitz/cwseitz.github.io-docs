...
which is clearly the same if we swap the order. It is similar for rotations:
...
\begin{align*}
\mathcal{D}\pi\ket{\bm{x}} &= \mathcal{D}\ket{-\bm{x}} = \ket{-\bm{x}'}\\
\pi\mathcal{D}\ket{\bm{x}} &= \pi\ket{\bm{x}'}= \ket{-\bm{x}'}
\end{align*}
...
because the momentum operator is odd under parity.
