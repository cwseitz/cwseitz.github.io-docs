...
\begin{align*}
S_{x}(t) &= S_{x}(0)\cos(\omega t)\\
S_{y}(t) &= S_{y}(0)\sin(\omega t)\\
S_{z}(t) &= S_{z}(0)
\end{align*}
...
To find the normalization constant $A$, we evaluate

\begin{align*}
A &= \left(\int_{-a}^{+a}\bra{\alpha}\ket{x}\bra{x}\ket{\alpha}dx\right)^{-1/2}\\
&= \left(\int_{-a}^{+a}(x-a)^{4}(x+a)^{4}dx\right)^{-1/2}\\
&= \frac{\sqrt{315}}{16 a^{9/2}}
\end{align*}
...
\begin{align*}
\langle (\Delta x)^{2}\rangle = \langle x^{2} \rangle - \langle x \rangle^{2} = \frac{a^{2}}{11}
\end{align*}
