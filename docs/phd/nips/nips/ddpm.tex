...
Point estimates $\hat{\bold{y}}_{i}$ produced by the DeepSTORM architecture lack uncertainty quantification. We imagine a Markov chain $\bold{y}\rightarrow \bold{x}\rightarrow \hat{\bold{y}}$. The input $\bold{x}$ contains information about the latent KDE $\bold{y}$, which under normal conditions, is insufficient for a perfect reconstruction $\hat{\bold{y}}$. Data processing inequality states $I(\bold{y}_k;\bold{x}) \geq I(\bold{y}_k;\bold{\hat{y}})$ or $H(\bold{y}_k|\bold{x}) \leq H(\bold{y}_k|\bold{\hat{y}})$, which tells us the entropy (uncertainty) of $p(\bold{y}_k|\bold{\hat{y}})$ is a upper bound on the entropy in $p(\bold{y}_k|\bold{x})$
