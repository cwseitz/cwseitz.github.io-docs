...
We consider datasets $(\bold{x}_i,\bold{y}_i,\hat{\bold{y}}_{i})_{i=1}^{N}$ of observed images $\bold{x}_i$ true kernel density estimate (KDE) images $\bold{y}_{i}$, and KDE estimates $\hat{\bold{y}}_{i}=\phi(\bold{x}_{i})$. Observations $\bold{x}_i$ are simulated under the likelihood (2). We aim to develop a framework for sampling from $p(\hat{\bold{y}}|\bold{x},\bold{y})$.

\section{Conditional Diffusion Model}

Point estimates $\hat{\bold{y}}_{i}$ produced by the DeepSTORM architecture lack uncertainty quantification. According to the data processing inequality 

\begin{equation*}
H(\bold{y}|\hat{\bold{y}}) \geq H(\hat{\bold{y}}|\bold{x}) + H(\bold{y}) - H(\hat{\bold{y}})
\end{equation*}


An upper bound on the quantity $H_{0} = H(\bold{y})-H(\hat{\bold{y}})$ can be found empirically by assuming a hypothetical model $\phi$ which perfectly models $\hat{\bold{y}}|\bold{x}$ s.t., $H(\hat{\bold{y}}|\bold{x})=0$. During training, one then proceeds by injecting the correct solution into a DDPM modeling $p(\bold{y}|\hat{\bold{y}})$. We then get that 

\begin{equation*}
H(\hat{\bold{y}}|\bold{x}) \leq H(\bold{y}|\hat{\bold{y}}) - H_{0}
\end{equation*}
