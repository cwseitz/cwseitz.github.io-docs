...
Point estimates $\hat{\bold{y}}_{i}$ produced by the DeepSTORM architecture lack uncertainty quantification. Consider this ``triangular'' Markov chain wherein the value of a KDE pixel $\bold{y}_{k}$ is latent in a low resolution image $\bold{x}$, and is inferred by a deep model: $\bold{y}_{k} \rightarrow \bold{x} \rightarrow \bold{\hat{y}}_{k} \rightarrow \bold{y}_{k}$ for which we find $p_{\phi}(\hat{\bold{y}}_{k}|\bold{x})p(\bold{y}_{k})=p_{\Psi}(\bold{y}_{k}|\hat{\bold{y}}_{k})p(\hat{\bold{y}}_{k})$. Therefore the entropy is

\begin{equation*}
h_{\Psi}(\bold{y}_{k}|\hat{\bold{y}}_{k}) = h_{\Phi}(\hat{\bold{y}}_{k}|\bold{x}) + \Delta h
\end{equation*}

where $\Delta h=h(\bold{y}_{k}) - h(\hat{\bold{y}}_{k})$. Evidently the entropy of the model prediction $h_{\Phi}(\hat{\bold{y}}_{k}|\bold{x})$, can be modeled by the entropy of our Gaussian diffusion model $h_{\Psi}(\bold{y}_{k}|\hat{\bold{y}}_{k})$. After training $\Psi$, one can measure $\Delta h$ by injecting the correct solution into the diffusion model modeling $\Psi$ and measure $\Delta h$, and thus compute $h_{\Phi}(\hat{\bold{y}}_{k}|\bold{x})$.
