...
Consider a probabilistic model with the following components:

\begin{itemize}
\item Continuous Coordinates: \( \Theta \)
\item Presence of Molecule: \( B \) (output from a neural network)
\item Total Number of Molecules: \( M \) (modeled by an HMM)
\item Binned Coordinates: \( Z \)
\end{itemize}

We aim to find the most probable configuration of these variables given the measured time series of photon counts \( X \). We maximize the posterior probability $P(\theta|X)$:

\begin{equation}
\theta_{\text{MAP}} = \underset{\Psi,\Phi}{\mathrm{argmax}}\; P(\theta|Z)P(Z|B,M)P_{\Psi}(B|X)P_{\Phi}(M|X)
\end{equation}


Optimization of $P(\theta|Z,X) = P(\theta|X)P(\theta|Z)$ can be obtained by sampling and using $P(\theta|Z)$ as a prior distribution. We can solve $ \underset{Z}{\mathrm{argmax}}\; P(Z|B,M)$ by LoG detection on $B$ and taking $M$ highest peaks in $B$.
