...
Pointillist SMLM data is quantified with Ripley's K-function $K(r)$, which counts events within a radius $r$. For $K(\sigma) > 0$, SMLM data can be crowded for some $\vec{k}$. On average 

\begin{equation*}
N_{s} = \lambda\langle\xi\rangle\;\;\; N_{b} = \lambda\langle\xi\rangle p_{\mathrm{ON}}K(\sigma)
\end{equation*}

where $\xi\sim P(\xi|\vec{k})$. $P$ might be a beta-distribution or normal approximating binomial for $N\rightarrow\infty$. Define $\zeta = 1/p_{\mathrm{ON}}K(\sigma)$ which we attempt to maximize with $K(\sigma)$ fixed and under the constraint that $p_{\mathrm{ON}}$ also achieves a desirable SR frame duration e.g., 1 second
