...
\begin{frame}{Dense localization with fluorescence antibunching}
\begin{itemize}
\item Pixels represent independent categorical variables, where the category is the number of photons collected
...
\end{itemize}
Suppose pixel $i$ collects $n_{i}$ photons and pixel $j$ collects $n_{j}$ photons. Call this pair $n$. We have,

\begin{equation*}
\Omega_{n} = P(X_{i}=n_{i},X_{j}=n_{j}) = \left[\sum_{\alpha}\left(\prod_{n}p_{i}^{\alpha_{n}}(1-p_{i})^{\alpha_{n}})\right)\right]\left[\sum_{\beta}\left(\prod_{n}p_{i}^{\beta_{n}}(1-p_{i})^{\beta_{n}})\right)\right]
\end{equation*}

where $\alpha,\beta$ are binary strings indicating which emitters contribute to the count. $\Omega_{n}$ has the dimension of the state space of $(X_{i},X_{j})$. Clearly,

\begin{equation*}
\langle X_{i}X_{j} \rangle = \sum_{x_{i},x_{j}}x_{i}x_{j}\Omega_{n}
\end{equation*}

It still isn't clear how to map $n\rightarrow (i,j)$ to make this computationally efficient
