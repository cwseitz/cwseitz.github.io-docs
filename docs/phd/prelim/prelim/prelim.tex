...
\begin{equation*}
\alpha = \int_{\delta}^{\Delta}\left(\sum_{n=0}^{\infty}Q(N=n)\psi(t|n;\vec{k})\right)dt
\end{equation*}

which is very hard to compute in terms of $\vec{k}$, so we generally resort to Monte Carlo estimates of $\alpha$.
