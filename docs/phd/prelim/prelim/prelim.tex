...
\begin{itemize}
\item Chromatin structure is dynamic and undergoes \textbf{phase separation}
$\rightarrow$ We are pushing the limits of SMLM for time-resolution
\end{itemize}
\end{frame}
...
\begin{frame}{DeepSTORM: An early approach to dense SMLM with deep learning}
...
\begin{frame}{Counting fluorescent molecules with a photon counting camera}
\begin{figure}
...
\begin{comment}
\begin{frame}{A hidden markov model for counting fluorescent molecules}
\begin{figure}
...
\begin{frame}
Suppose we have a model $\mathcal{M} = (S,T)$ with a spatial part $S$ and temporal part $T$. We want to compare different models, based on the ratio $P(\mathcal{M}_{1}|D)/P(\mathcal{M}_{2}|D)$.

\begin{equation*}
\frac{P(\mathcal{M}_{1}|D)}{P(\mathcal{M}_{2}|D)} = \frac{P(D|\mathcal{M}_{1})P(\mathcal{M}_{1})}{P(D|\mathcal{M}_{2})P(\mathcal{M}_{2})}
\end{equation*}

Therefore, we need to find $P(D|\mathcal{M}_{n})$. If we assume the two model components are independent

\begin{equation*}
P(D|\mathcal{M}_{n}) = P(D|S_{n})P(D|T_{n})
\end{equation*}

We can use the HDP-HMM model to integrate out $\theta$ with MCMC:

\begin{equation*}
P(D|T_{n}) = \int P(D,\theta | T_{n}) d\theta
\end{equation*}

Then we use a Langevin Monte Carlo (LMC) sampler for the spatial model

\begin{equation*}
P(D|S_{n}) = \int P(D,\phi | S_{n}) d\phi
\end{equation*}

Both of these sampling methods also provide the MAP estimates of the parameters $\theta,\phi$.
