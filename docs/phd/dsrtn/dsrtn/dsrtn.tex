...
Thus we have shown that each neighborhood can be considered independently of every other and we only need to sum over the subspace spanned defined by $Y$. Remembering that we actually need a likelihood over times series  $P(\vec{H}|\Theta)$, we hypothesize that pairwise and higher order temporal correlation structure of a pixel neighborhood $\mathcal{N}$ over the series of frames $Y_{n}$ which satisfy 
