...
\begin{align*}
P(H_{t}|\Theta) &= \prod_{t} \sum_{\vec{X}} P(H_{t}|\vec{X},\Theta)P(\vec{X}|\Theta)
\end{align*}



where we have used the conditional independence assumptions of the HMM to factorize $P(H_{t},\vec{X}|\Theta)$ into $P(H_{t}|X,\Theta)$ and $P(\vec{X}|\Theta)$. The sum above is typically intractable - a problem which we will address in the next section. $P(H_{t}|\vec{X},\Theta)$ is the likelihood function of the observed data $H_t$ given the hidden state $X_t$ and the model parameters $\Theta$, which is a Poisson distribution with rate parameter that depends on $X_t$. $P(\vec{X}|\Theta)$ will be determined by the transition probabilities and the initial state probabilities of the HMM. 
