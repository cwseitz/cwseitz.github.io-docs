...
The matrix $T$ is not necessarily symmetric i.e. $T_{ij} \neq T_{ji}$ and is defined in the \emph{forward direction}. One should note that the columns $T_{j}$ \emph{do not} define a probability distribution $P(X_{t} = \omega_{i} | X_{t-1} = \omega_{j})$ and therefore do not sum to unity. The probability $P(X_{t} = \omega_{i} | X_{t-1} = \omega_{j})$ has no meaning in this context, since by definition, time is flowing in the positive direction. Of course, we could write $T^T$ such that time flows backwards, which is important in the theory of detailed balance, but we do not introduce this here. We simply sample $X_{t} \sim P(X_{t} = \omega_{j} | X_{t-1} = \omega_{i})$, assign $i=j$, and repeat. It directly follows from the fundamental rules of probability, the first order dynamics for a \textbf{particular} state $\omega_{i}$: $P(\omega_{i},t)$ is given by
