...
\subsection{Bayesian Inference of PSF parameters and the Cramer-Rao bound}

Suppose we are given samples from a model distribution with unknown parameters. This model defines a joint likelihood of the samples at each point in $\theta$ space, formally the posterior $P(X|\theta)$. We may then ask - does the likelihood of the data vary as we vary the parameters? If the likelihood surface is flat, all parameter sets would be equally likely and the data does not appear to carry much information about the parameters. Perhaps our model is complete nonsense, and the data simply cant carry substantial information about the parameters. Moreover, if the surface has a number of bumps or inflection points, then we expect that maybe some parameter sets are more likely that others. Fisher information, relates this ``bumpiness" of the likelihood surface to a precise metric of \emph{information} carried by the data about the parameters, and is measured by computing the variance the second derivative of the log-likelihood over the parameter space\\


The log-likelihood of an image under parameters $\theta$, assuming each pixel is an independent random variable, reads

\begin{align*}
\ell(H|\theta) = \log\prod_{k}^{M^{2}} P(H_{k}|\theta) = \sum_{k=1}^{M^{2}} \ell (H_{k}|\theta)
\end{align*}

This is fairly simple when the likelihood is Gaussian


\begin{equation*}
\ell(H|\theta) = \frac{1}{2}\sum_{k} \log 2\pi\sigma_{k}^{2} + \frac{(H_{k}-\mu_{k})^{2}}{\sigma_{k}^{2}}
\end{equation*}
