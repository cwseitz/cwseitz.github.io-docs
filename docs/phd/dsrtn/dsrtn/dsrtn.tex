...
This is a daunting sum over an enormous state space $\vec{X}$. However, it is reasonable to propose that the joint distribution over a frame will factor into image neighborhoods due to locality. Therefore, we define the \emph{pixel assignment} $S$ such that $S(x,y)$ is assigned a label designating the fluorophore or combination of fluorophores collected photons were emitted by. The probability of an assignment given the particle locations is definite i.e., $P(S|\Theta) = \delta(S)$ where each pixel is attributed a \emph{labeling set}

\begin{equation*}
S(x,y) =  \{\mathbbm{1}\left(\lambda_{n}(x,y) > \epsilon\right)\}_{n=1}^{N}
\end{equation*}
 
for some minimum rate tolerance $\epsilon << 1$. The neighborhood assignment $S$ can be absorbed into the parameter set and included as part of the inference problem. We can now see that the neighborhood $\mathcal{N}_{k} = \{H_{1},...,H_{N}\}$ per particle includes all pixels which detected photons from fluorophore $k$. When conditioned on $S$, the likelihood will factor into several smaller joint distributions. For example,

\begin{align*}
P(\vec{H}|\Theta,S) &= \prod_{k}P(\mathcal{N}_{k})
\end{align*}

Notice that each neighborhood $\mathcal{N}_{k}$ is not guaranteed to contain only one fluorophore. Indeed, it may contain multiple, and those that contain multiple have a neighborhood assignment $S_{\mathcal{N}}$ with at least one pixel that contains more than one label. Allowing this multiplicity makes it possible to factor the joint distribution over images while drastically simplifying marginalization of the hidden state and preserving the spatial structure of the image. To see this, we write

\begin{align*}
P(\vec{H}|\Theta,S) &= \prod_{k}P(\mathcal{N}_{k}|\Theta,S) \\
&= \prod_{k}\sum_{X}P(\mathcal{N}_{k},X|\Theta,S) \\
&= \prod_{k}\sum_{x}P(\mathcal{N}_{k}|X=x,\Theta,S)P(X=x|\Theta,S)
\end{align*}

To make this sum more tractable, we now leverage the conditional independence structure embedded in $S$. That is, $S$ implies that each neighborhood $\mathcal{N}_{k}$ depends only on a small subset of $X$, so we can define $Y$ as the partition of $X$ that it depends on and $Z$ the partition it does not. This implies that $P(\mathcal{N}|X) = P(\mathcal{N}|Y,Z) = P(\mathcal{N}|Y)$. Therefore,
