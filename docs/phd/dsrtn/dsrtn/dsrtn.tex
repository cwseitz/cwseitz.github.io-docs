...
where $t = \frac{x}{\sqrt{2}\sigma}$. Of course, this can be used to construct the integral over an arbitrary interval $[a,b]$

\begin{equation*}
\mathrm{erf}(b) - \mathrm{erf}(a) = \frac{2}{\sqrt{\pi}}\int_{\frac{a}{\sqrt{2}\sigma}}^{\frac{b}{\sqrt{2}\sigma}} \exp(-t^{2})dt
\end{equation*}

We use this result to compute the Poisson rate $\lambda_{k}$ at pixel $k$. Start with
...
Let $t = (x-x_{0})/\sqrt{2}\sigma$ and $z = (y-y_{0})/\sqrt{2}\sigma$

\begin{align*}
\lambda_{k} &= \frac{1}{2\pi\sigma^{2}}\left(\int_{t_{k}}^{t_{k+1}}\exp\left(-t^{2}\right)dx\right)\left(\int_{z_{k}}^{z_{k+1}}\exp\left(-z^{2}\right)dy\right)\\
&= \frac{2}{\pi^{2}\sigma^{2}}\left[\mathrm{erf}(t_{k+1}) - \mathrm{erf}(t_{k})\right]\left[(\mathrm{erf}(z_{k+1}) - \mathrm{erf}(z_{k})\right]
\end{align*}
