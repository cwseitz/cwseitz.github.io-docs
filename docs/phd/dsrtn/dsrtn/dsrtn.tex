...
since the the convolution of two Poisson distributions is Poisson. Under this approximation, the model log-likelihood is

\begin{align*}
\ell(\vec{H}) &= \log \prod_{k} \frac{e^{-\left(g_{k}\mu_{k} + \sigma_{k}^{2}\right)}\left(g_{k}\mu_{k} + \sigma_{k}^{2}\right)^{n_{k}}}{n_{k}!}\\
&= \sum_{k} n_{k}\log\left(g_{k}\mu_{k} + \sigma_{k}^{2}\right) - \log n_{k}! - g_{k}\mu_{k} - \sigma_{k}^{2}
\end{align*}
