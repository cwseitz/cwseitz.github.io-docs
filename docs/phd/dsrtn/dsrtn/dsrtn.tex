...
where we have used $J_{\mu}$ to represent the jacobian of $T$ and $J_{\ell}$ for the jacobian of $W$. Similar notation is used for the corresponding Hessian matrices. We have already computed $J_{\mu}$ and now for $J_{\ell}$ and $H_{\ell}$ we write
...
respectively. Computing $H_{\mu}$ is a bit more involved. 
