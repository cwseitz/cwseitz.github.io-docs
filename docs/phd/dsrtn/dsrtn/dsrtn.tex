...
\begin{align*}
\nabla_{\theta}\mathrm{ELBO} = - \underset{{\theta \sim q(\theta)}}{\mathbb{E}}\left(\nabla_{\theta}\log q(\theta) - \nabla_{\theta}\log p(x|\theta)\right)
\end{align*}

To compute the gradients of the ELBO, we recall the following pair of equations from the previous section on the multi-emitter regime

\begin{align*}
\mu_{k}' &= g_{k}\eta\Delta\left(\sum_{j}N_{j}\lambda_{\theta_{j}}(x_{k},y_{k}) + B_{0}\right)+ \sigma_{k}^{2}\\
\ell(\vec{H}|\theta) &= \sum_{k}  \log n_{k}! + \mu_{k}' - n_{k}\log\left(\mu_{k}'\right)
\end{align*}

The gradients are given by the chain rule $\frac{\partial \ell}{\partial\theta_{j}} = \frac{\partial \ell}{\partial\mu_{j}}\frac{\partial \mu}{\partial\theta_{j}}$
