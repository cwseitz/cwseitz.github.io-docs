...
The Fisher Information matrix depends on the parametrization chosen, and, in general, if we rewrite our model in terms of some other parameter $\eta$, then by the vector chain rule $\nabla_{\theta} = J^{T}\nabla_{\eta}J$ where $J$ denotes the Jacobian matrix with components $J_{ij} = \partial \eta_{i}/\partial \theta_{j}$. The Fisher information becomes $I(\theta) = J^{T}I(\eta)J$.
...
which (I think) can be derived from using the note on reparameterization above. 

In this context, the mean vector is $\mu = o + \lambda\tau$ and the covariance matrix is diagonal: $\mathrm{diag}(\Sigma) = \lambda\tau + \sigma^{2}$. But we can always standarize each variable by subtracting out $o$ and divide by $\sigma$ for a particular pixel. Let $\mu = \lambda\tau$

\begin{align*}
\frac{\partial\mu}{\partial x_{0}} = \tau\lambda_{y}\left(\frac{\sqrt{\frac{2}{\pi }} e^{-\frac{\left(-\frac{a}{2}+x_{k}-\text{x0}\right)^2}{2 \sigma ^2}}}{\sigma }-\frac{\sqrt{\frac{2}{\pi }} e^{-\frac{\left(\frac{a}{2}+x_{k}-\text{x0}\right)^2}{2 \sigma ^2}}}{\sigma }\right)
\end{align*}

\subsection{Poisson case}

But we can always standarize each variable by subtracting out $o$ and divide by $\sigma$ for a particular pixel (which can be directly measured). Then each pixel is a Poisson random variable.
