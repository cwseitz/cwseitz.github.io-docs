...
We would like to perform inference on the number of particles in the time-series as well as their respective positions and switching rate constants. Here, I will take a bottom up approach and describe how a time-series is generated in order to construct the full posterior for these bundled parameters $\Theta$ given the image sequence. Consider first $K$ emitters cycling through the four-state Markov process presented in the previous section. The photon emission rate in each of the four states is given by $\vec{s} = \left(\mu_{0},0,0,0\right)$. If we know the hidden state $X_{t}$ at some time $t$, then $\vec{N}(t) \sim \mathrm{Poisson}(X_{t}\vec{s})$. As shown previously, the rate at each pixel must be

\begin{equation*}
\vec{\mu}_{t}(x_{i},y_{i}) = \eta\Delta t\left(\lambda_{\theta}(x_{i},y_{i})\vec{N}_{t,i} + \vec{B}_{t}(x_{i},y_{i})\right)
\end{equation*}

The image we then observe $H_{t}$ is given by sampling from the distribution in (1.6) or its Poisson approximation. To now see how we infer $\Theta = (\vec{k},\theta)$ from the image sequence $\vec{H}$, we write the posterior on $\Theta$ according to Bayes rule

\begin{align*}
P(\Theta|\vec{H}) = \frac{P(\Theta)\prod_{t}P(H_{t}|\Theta)}{P(\vec{H})}
\end{align*}

Let us first address the likelihood function. To account for the unknown hidden state $X_{t}$, we need to marginalize over it. We can use the law of total probability to do this:

\begin{align*}
P(H_{t}|\Theta) &= \sum_{\vec{X}} P(H_{t},\vec{X}|\Theta) \\
&= \sum_{\vec{X}} P(H_{t}|\vec{X},\Theta)P(\vec{X}|\Theta)
\end{align*}


where we have used the conditional independence assumptions of the HMM to factorize $P(H_{t},\vec{X}|\Theta)$ into $P(H_{t}|X,\Theta)$ and $P(\vec{X}|\Theta)$. Therefore, the likelihood reads

\begin{align}
P(\vec{H}|\Theta) &= \prod_{t} \sum_{\vec{X}_{t}} P(H_{t}|\vec{X}_{t},\Theta)P(\vec{X}_{t}|\Theta)
\end{align}

At first glance, this likelihood appears intractable due to the enormous size of the state space of $\vec{X}$. However, barring background-induced correlations, pixels are conditionally independent given the hidden state, while locality suggests that each pixel is only dependent on a small subset of the hidden state $X$. Dropping the time index for clarity,

\begin{align*}
P(H|X) &= P(H_{1}|X)P(H_{2}|X)...P(H_{K}|X) \\
&= P(H_{1}|X_{c_{1}})P(H_{2}|X_{c_{2}})...P(H_{K}|X_{c_{K}})\\
&= \prod_{k}P(H_{k}|X_{c_{k}})
\end{align*}

where $X_{c_{k}}$ denotes a small subset of $X$. Thus we can rewrite the likelihood as

\begin{align}
P(\vec{H}|\Theta) &= \prod_{t} \sum_{X_{c_{k}}} \prod_{k}P(H_{k}|X_{c_{k}})P(X_{c_{k}}|\Theta)
\end{align}

We now see that the summation only needs to be evaluated over the state space of $X_{c_{k}}$ which consists of at most a few hidden variables (emitters), making the likelihood tractable. Again, $P(H_{k}|X_{c_{k}},\Theta)$ is just a Poisson distribution according to our approximation in (1.6).

\begin{align}
\log P(H_t|\vec{X},\Theta) \rightarrow \log P(H_t|\vec{X},\Theta) - \lambda \sum_{i,j} w_{ij} \left| H_{t,i} - H_{t-1,j} \right|,
\end{align}

where $\lambda$ is a hyperparameter that controls the strength of the penalty, $w_{ij}$ are the pairwise weights between neighboring pixels $i$ and $j$, and $H_{t,i}$ and $H_{t-1,j}$ are the intensities of pixel $i$ at time $t$ and pixel $j$ at time $t-1$, respectively. The sum is taken over all pairs of neighboring pixels in the image.


\subsection{Incorporating a Dirichlet stick-breaking prior}

\begin{align*}
P(\Theta|\vec{H}) = \frac{\mathrm{Dir}(\Theta|a,b)\prod_{t}\sum_{\vec{X}} P(H_{t}|\vec{X},\Theta)P(\vec{X}|\Theta)}{P(H)}
\end{align*}
