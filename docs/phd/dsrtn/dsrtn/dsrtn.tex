...
The reparameterization trick works by transforming the random variable $\theta$ in the variational distribution $q_\phi(\theta)$ into a new random variable $\epsilon$ with a fixed distribution $p(\epsilon)$, such that $\theta = g_\phi(\epsilon)$ for some differentiable function $g_\phi$. Then, we can write:
