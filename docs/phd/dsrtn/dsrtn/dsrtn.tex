...
which is a valid approximation in the limit of large $\sigma_{k}^{2}$. Therefore, we can write the distribution over $H_{k}$ as

\begin{align*}
P(H_{k}- o_{k} + \sigma_{k}^{2}) &= P(S_{k} + \xi_{k} - o_{k} + \sigma_{k}^{2})\\
&= P(S_{k}) \circledast P(\xi_{k}- o_{k} + \sigma_{k}^{2})\\
&= \mathrm{Poisson}\left(g_{k}\mu_{k} - o_{k} + \sigma_{k}^{2}\right)
\end{align*}


since the convolution of two Poisson distributions is Poisson. In the Poisson approximation, the model log-likelihood is surprisingly simple
