...
This expression is difficult to use in practice, due to the infinite sum, so we can leverage the convolution theorem. It is substantially faster to first evaluate the Fourier transform of $P(H_{K}|S_{k})$ and $P(S_{k})$, take their product, and inverse transform to get the desired density function. Dropping the pixel subscripts for the moment, the Fourier transform of a Gaussian reads
...
\begin{align*}
\tilde{P}(S) &= \int_{-\infty}^{\infty} \frac{e^{-\lambda}\lambda^{x}}{x!}e^{-i\omega x}dx\\
&= \sum_{n=0}^{\infty} \frac{e^{-\lambda}\lambda^{n}}{n!}e^{-i\omega n}\\
&= e^{-\lambda}\sum_{n=0}^{\infty} \frac{\left(e^{i\omega}\lambda\right)^{n}}{n!}\\
&= e^{{\lambda}(e^{i\omega}-1)}
\end{align*}

\begin{align*}
P(H) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty} e^{-\frac{\sigma_{w} ^2 \omega ^2}{2}+i \mu  \omega } e^{{\lambda}(e^{i\omega}-1)}e^{-i\omega x} d\omega
\end{align*}
