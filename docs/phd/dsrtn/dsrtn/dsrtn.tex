...
At first glance, this likelihood appears intractable due to the enormous size of the state space of $\vec{X}$. However, barring background-induced correlations, pixels are conditionally independent given the hidden state, while locality suggests that each pixel is only dependent on a small subset of the hidden state $X$. Suppose we assume that the joint distribution over a single frame will factor into neighborhoods: $P(H) = \prod_{\mathcal{N}}P(\mathcal{N})$.

\begin{align*}
P(\mathcal{N}) &= \sum_{X}P(\mathcal{N},X)\\
&= \sum_{X}P(\mathcal{N}|X)P(X)\\
\end{align*}

\begin{align*}
P(\mathcal{N}|X) &= P(\mathcal{N}_{1}|X)P(\mathcal{N}_{2}|X)...P(\mathcal{N}_{K}|X) \\
&= P(\mathcal{N}_{1}|X_{c_{1}})P(\mathcal{N}_{2}|X_{c_{2}})...P(\mathcal{N}_{K}|X_{c_{K}})\\
&= \prod_{k}P(\mathcal{N}_{k}|X_{c_{k}})
\end{align*}

Therefore,

\begin{align*}
P(\mathcal{N}) &= \sum_{X}P(X)\prod_{k}P(\mathcal{N}_{k}|X_{c_{k}})\\
&= \sum_{X}\prod_{i}P(X_{i})\prod_{k}P(\mathcal{N}_{k}|X_{c_{k}})\\
\end{align*}

Now notice that $P(\mathcal{N}_{k}|X_{c_{k}})$ will be constant over the terms of the sum where $X_{c_{k}}$ is constant. Therefore, we can rewrite the summation as

\begin{align*}
P(\mathcal{N}) &= \sum_{X}\prod_{i}P(X_{i})\prod_{k}P(\mathcal{N}_{k}|X_{c_{k}})\\
&= \sum_{Y}\left(\prod_{k}P(\mathcal{N}_{k}|X_{c_{k}})\sum_{X} \prod_{i}P(X_{i})\right)\\
&= \sum_{Y}\prod_{k}P(\mathcal{N}_{k}|X_{c_{k}})
\end{align*}

where $Y$ is a unique state in the subspace $X_{c_{k}}$.
