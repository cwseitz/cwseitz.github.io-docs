...
However this noise model is incomplete, because detectors often suffer from dark noise (thermal noise). We define another r.v. $W_{k}$ which represents Gaussian dark noise associated with pixel $k$. Unless otherwise specified we will always assume that $W_{k} \sim \mathcal{N}(m_{k},\sigma_{w,k}^{2})$. We represent our corrupted signal as another random variable $H_{k} = S_{k} + W_{k}$. The r.v. $S_{k}$ has units of photons $[\mathrm{p}]$, while $H_{k}$ and $W_{k}$ have units of photoelectrons $[e^{-}]$.  is the gain of the detector element $g_{k}\; [e^{-}\mathrm{p}^{-1}]$. A fundamental result in probability theory is that the distribution of $H_{k}$ is the convolution of the distributions of $S_{k}$ and $W_{k}$,
