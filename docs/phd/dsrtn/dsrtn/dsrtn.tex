...
\subsection{A Hidden Markov Model for super-resolution time-series}
...
However, it is reasonable to assume that the joint distribution over a single frame will factor into neighborhoods: $P(H) = \prod_{\mathcal{N}}P(\mathcal{N})$ due to locality in the image. The distribution on each neighborhood $\mathcal{N}$ is

\begin{equation*}
P(\mathcal{N}) = \sum_{X}P(\mathcal{N},X) = \sum_{x}P(\mathcal{N}|X=x)P(X=x)\\
\end{equation*}
...
This will simplify even further if we note that we can group terms since $P(\mathcal{N}|Y)$ is constant over states which share the same substate $y$
...
Thus we have shown that each neighborhood can be considered independently of every other and we only need to sum over the subspace spanned defined by $Y$.


\subsection{Generalizations to time-series data}
