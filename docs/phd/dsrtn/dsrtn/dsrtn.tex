...
\begin{equation*}
\mathrm{erf}(x) = \frac{2}{\sqrt{\pi}}\int_{0}^{x} \exp(-\alpha^{2})dx
\end{equation*}

where $\alpha = \frac{x}{\sqrt{2}\sigma}$. Of course, this can be used to construct the integral over an arbitrary interval $[a,b]$

\begin{equation*}
\mathrm{erf}(b) - \mathrm{erf}(a) = \frac{2}{\sqrt{\pi}}\int_{a}^{b} \exp(-\alpha^{2})dx
\end{equation*}

We use this result to compute the Poisson rate $\lambda_{k}$ at pixel $k$

\begin{align*}
\lambda_{k} &= \frac{1}{2\pi\sigma^{2}}\left(\int_{x_{k}}^{x_{k+1}}\exp\left(-\frac{(x-x_{0})^{2}}{2\sigma^{2}}\right)dx\right)\left(\int_{y_{k}}^{y_{k+1}}\exp\left(-\frac{(y-y_{0})^{2}}{2\sigma^{2}}\right)dy\right)\\
&= \frac{1}{2\pi\sigma^{2}}\left(\int_{x_{k}}^{x_{k+1}}\exp\left(-\frac{(x')^{2}}{2\sigma^{2}}\right)dx\right)\left(\int_{y_{k}}^{y_{k+1}}\exp\left(-\frac{(y')^{2}}{2\sigma^{2}}\right)dy\right)\\
&=\frac{1}{2\pi\sigma^{2}}\frac{\pi}{4}\left(\mathrm{erf}(x_{k+1}\sqrt{\alpha})-\mathrm{erf}(x_{k}\sqrt{\alpha})\right)\left(\mathrm{erf}(b_{k+1}\sqrt{\alpha})-\mathrm{erf}(b_{k}\sqrt{\alpha})\right)
\end{align*}
