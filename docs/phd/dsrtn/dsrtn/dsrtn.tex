...
We adopt a common Fisher information theoretic view of localization microscopy, which provides a rigorous statistical framework for bounding our uncertainty in SMLM measurements. This involves defining a Cramer-Rao lower bound on the uncertainty in the localization parameters - typically lateral or lateral and axial coordinates of each of $K$ emitters in the field of view. Many models estimate the CRLB using point spread functions in continuous space; however, the spatial binning process  results in loss of information during photon detection, which, in turn, can inflate our uncertainty. Therefore, in the remainder of this chapter, I develop and utilize a \emph{generative image model} rather than using calculus on continuous spaces to compute uncertainties. In addition, I will show that, in the isolated emitter regime, the posterior distribution is well approximated by a multivariate normal distribution around the maximum likelihood estimate (MLE). Armed with this Bayesian approach, we pose the question: can we (even in principle) reduce our uncertainty in localization parameters using temporal fluctuations? That is, does a time series $\vec{H}$ carry any \emph{information} that an estimator $\mathcal{E}$ can use to infer the number of fluorophores and their coordinates more precisely?

\section{Generative models for single molecule localization microscopy}

\subsection{Symmetric Gaussian point spread function}

Most detectors used for imaging have many elements (pixels) so that we can record an image projected onto the detector by a system of lenses. Due to diffraction, any point emitter, such as a single fluorescent molecule, will be registered as a diffraction limited spot. It is common to describe the point spread function as a two-dimensional symmetric Gaussian:

\begin{equation}
\mathrm{G}(x,y) = \frac{1}{2\pi\sigma^{2}}e^{-\frac{(x-x_{0})^{2}+(y-y_{0})^{2}}{2\sigma^{2}}} + B(x,y)
\end{equation}
...
\subsection{Astigmatic Gaussian point spread function}

In 2D SMLM simulations, a 2D PSF  model  with  a z-dependent  isotropic  width $\sigma$ can  be  used. For 3D, we could use that the isotropic Gaussian point spread function has a FWHM $\sigma$ which is dependent on the axial coordinate. However, it can be shown that the error around the focus is very large and negative and positive defocus cannot be distinguished given the symmetric dependence in $z$.  Therefore, for 3D SMLM, a simple approach is to introduce astigmatism into the detection path using a weak ($f\approx 10\mathrm{m}$) cylindrical lens. This gives an anisotropic Gaussian point spread function which is elongated perpendicular to the optical axis, depending on the axial ($z$) position of the fluorescent emitter. Nevertheless, this point spread function remains very simple:

\begin{equation}
\mathrm{G}(x,y) = \frac{1}{2\pi\sigma^{2}}e^{-\frac{(x-x_{0})^{2}}{2\sigma_{x}^{2}}+\frac{(y-y_{0})^{2}}{{2\sigma_{y}^{2}}}} + B(x,y)
\end{equation}

The pixel dependent rates directly follow from our analysis of the istropic case, adjusting for the elongation in $x$ or $y$

\begin{align*}
\lambda_{x}(x) &= \frac{1}{2}\left(\mathrm{erf}\left(\frac{x+a/2-x_{0}}{\sqrt{2}\sigma_{x}}\right) -\mathrm{erf}\left(\frac{x-a/2-x_{0}}{\sqrt{2}\sigma_{x}}\right)\right)\\
\lambda_{y}(y) &= \frac{1}{2}\left(\mathrm{erf}\left(\frac{y+a/2-y_{0}}{\sqrt{2}\sigma_{y}}\right) -\mathrm{erf}\left(\frac{y-y/2-y_{0}}{\sqrt{2}\sigma_{y}}\right)\right)
\end{align*}

What remains to be shown here is the relationship between $\sigma_{x}$, $\sigma_{y}$ and the axial position $z$. 


\subsection{Shot noise and readout noise}
...
\begin{equation*}
H_{(\ell,\theta)} = J_{(\mu,\theta)}^{T} H_{(\ell,\mu)} J_{(\mu,\theta)} + (J_{(\ell,\mu)}\otimes I_{n})H_{(\mu,\theta)}
\end{equation*}

which we will from now on abbreviate as 

\begin{equation}
\bm{H} = J_{2}^{T} H_{1} J_{2} + (J_{1}\otimes I_{n})H_{2}
\end{equation}
...
\begin{align*}
H_{k}' &\sim \mathrm{Poisson}(\mu_{k}')\\
\mu_{k}' &= g_{k}\eta\Delta\left(\sum_{j=1}^{K}N_{j}\lambda_{\theta_{j}}(x_{k},y_{k}) + B_{0}\right)+ \sigma_{k}^{2}
\end{align*}

Suppose we have $K$ particles within the field of view. Assuming that $\sigma$ is not fixed, we then have $4K$ total parameters and the Hessian matrix becomes a $4K\times 4K$ matrix. To compute that matrix, notice that $J_{1}$ and $H_{1}$ have not changed relative to the single emitter case. However, $J_{2}$ and $H_{2}$ need to be expanded to contain gradients of $\vec{\mu}$ w.r.t all $4K$ parameters. This is not hard, since a taking gradients w.r.t a parameter for particle $j$ call it $\theta_{j}$ will delete all terms in the sum except the $j$th term. This suggests that $H_{2}$ is block diagonal, which one block per particle. The structure of $J_{2}$ is also straightforward, we just add a row for each particle, and columns $4j$ to $4(j+1)$ are populated with the necessary gradients for that particle.
