...
In one approach to this issue, deep generative models have been used to learn the generating distribution of images of cellular structures from super-resolution data. Trained models can then be used to predict super-resolution images based on sparse localizations or widefield images (Ouyang 2018; Barth 2020; Chen 2023). Other approaches leverage convolutional neural networks (CNNs) to transform dense images into a localization map (Nehme 2020; Speiser 2021). In general, convolutional neural networks are well-known for their ability to discover higher order spatial structure in an image to infer latent variables. In essence, this can be seen as learning a translation from SMLM image intensities to pixel-wise probabilities of emitter occupancy. This second approach shows great promise for dense SMLM but suffers from two major drawbacks. First, CNNs are often unconstrained by physical variables, such as the total number of fluorescent emitters in a given region of interest. Second, CNNs do not explicitly model the likelihood and therefore do not allow direct computation of localization uncertainty. Therefore, we introduce a novel inference scheme which (i) Infers the number fluorescent emitters in the ROI by modeling photon statistics measured with a photon counting camera (ii) Permits direct computation of localization uncertainty using a gradient-based Markov Chain Monte Carlo (MCMC) scheme, known as Langevin dynamics.
