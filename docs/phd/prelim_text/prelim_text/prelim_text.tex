...
\end{center}
\caption{\textbf{SPAD cameras have lower sensitivity compared to CMOS}. (left) Quantum dots imaged with a SPAD camera using 100X/1.4NA oil immersion objective with continous-wave 640nm laser excitation. (left inset) Peak photon counts of isolated QDs using SPAD. (right) Quantum dots imaged with a CMOS camera using 60X/1.4NA oil immersion objective with continous-wave 640nm laser excitation. (right inset) Peak photon counts of isolated QDs using CMOS (right)}
\end{figure}

\captionof{figure}{\textbf{Super-resolution of nucleosome-BRD4 interactions in living cells}. (A) Density and pointillist representation of nucleosome organization in a living cell recovered with MLE (B) Principle of Besag's L-Function for cluster analysis (C) Two color image with super-resolved nucleosomes and diffraction limited BRD4 signal. (D) Average L-function for living Hela cells exposed to 5 percent 1,6 Hexanediol or 1uM JQ1 for 8h. Envelope drawn at one standard deviation}
...
\begin{align*}
\theta_{\text{MAP}} &= \underset{\Psi}{\mathrm{argmax}}\; P(\theta|X)\\
&= \underset{\Psi}{\mathrm{argmax}}\; P(\theta|\theta_{0})P(\theta_{0}|B,M)P_{\Psi}(B|S)P(M|X)
\end{align*}
