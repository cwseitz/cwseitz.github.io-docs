...
First order derivatives of the above sum can often be computed analytically, depending on the spatial function $\mu$. The Poisson approximation is also convenient for computing the Cramer-Rao lower bound, which bounds the uncertainty of a statistical estimator from below. This calculation was first written down in (Smith 2010), so we simply write the major result here
...
The Cramer-Rao lower bound on parameter unceratainty is then $\mathrm{CRLB} = \sqrt{I^{-1}}$.
...
\subsection{Enhancing image resolution with deep generative modeling}

\clearpage
\begin{figure}
\begin{center}
\includegraphics[width=14cm]{Diffusion.png}
\end{center}
\caption{}
\end{figure}

\subsection{Application: visualizing nucleosome cluster dynamics in BRD4 condensates}
...
\begin{figure}
\begin{center}
\includegraphics[width=14cm]{Histones.png}
\end{center}
\caption{}
\end{figure}
