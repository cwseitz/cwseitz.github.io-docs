...
The usual procedure is then to learn a parametric representation of the \emph{reverse process}, and therefore generate samples from  $p(\bold{y}_{0})$, starting from noise. Formally, $p_{\Psi}(\bold{y}_{0}|\bold{\hat{y}}) = \int p_{\Psi}(\bold{y}_{0:T}|\bold{\hat{y}})d\bold{\hat{y}}_{1:T}$ where $\bold{y}_{t}$ is a latent representation with the same dimensionality of the data.  $p_{\Psi}(\bold{y}_{0:T}|\bold{\hat{y}})$ is a Markov process, starting from a noise sample $p_{\Psi}(\bold{y}_{T}) = \mathcal{N}(0,\bold{I})$. 

\begin{equation}
p_{\Psi}(\bold{y}_{0:T}) = p_{\Psi}(\bold{y}_{T})\prod_{t=1}^{T} p_{\Psi}(\bold{y}_{t-1}|\bold{y}_{t}) \;\;\; p_{\Psi}(\bold{y}_{t-1}|\bold{y}_{t}) = \mathcal{N}\left(s_{\Psi}(\bold{y}_{t}),\beta_{t}I\right)
\end{equation}
...
\begin{equation}
\mathcal{L}_{\Psi} = \mathbb{E}\left[-\log p_{\Psi}(\bold{y}_{0}|\bold{x})\right] \leq  \mathbb{E}\left[-\log \frac{p_{\Psi}(\bold{y}_{0:T}|\bold{x})}{q(\bold{y}_{1:T}|\bold{y}_{0})}\right]
\end{equation}
...
\begin{equation}
\Psi^{*} = \underset{\Psi}{\mathrm{argmin}} \underset{(\bold{\hat{y}},\bold{y}_{0})}{\mathbb{E}}\underset{(\epsilon,\gamma)}{\mathbb{E}} \left[ s_{\Psi}\left(x, \sqrt{\gamma} \bold{y}_0 + \sqrt{1 - \gamma} \epsilon \, \middle|\, \bold{y}_{t}, \gamma\right) - \epsilon \right],
\end{equation}

After training, samples can be generated by 

\begin{equation}
\bold{y}_{t-1} = \frac{1}{\sqrt{1-\beta_{i}}}\left(\bold{y}_{i} + \beta_{i}s_{\Psi}(\bold{y}_{t})\right) + \sqrt{\beta_{i}}\bold{\xi}
\end{equation}
