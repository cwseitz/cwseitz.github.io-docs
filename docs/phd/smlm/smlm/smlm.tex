...
\begin{frame}{Generalizing to a variable number of particles}

\begin{align*}
P(k,\theta | H) = \frac{P(X|k,\{\theta\}_{i=1}^{k})P(\theta|k)P(k)}{P(H)}
\end{align*}

where $P(k)$ is a hyperprior. One option is to use the Dirichlet process

\begin{align*}
P(k,\{\theta\}_{i=1}^{k} | H) &= \frac{P(H|k,\{\theta\}_{i=1}^{k})DP(\alpha,P(\theta|k))}{P(H)}
\end{align*}
