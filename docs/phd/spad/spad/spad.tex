...
Define $g^{(2)}(0) = \frac{G^{(2)}(0)-B}{\langle G^{(2)}(m)\rangle -B}$ where $B = \langle x_{\mathrm{background}}\rangle = N_{\mathrm{frames}}\lambda\zeta$ is the expected number of background-signal coincidences in the acquired ROI 

\begin{equation*}
G^{(2)}(0) \sim \mathrm{Binomial}(N_{\mathrm{frames}}, P(\bold{x}_{t} \geq 2))
\end{equation*}

\begin{equation*}
G^{(2)}(m) \sim \mathrm{Binomial}(N_{\mathrm{frames}}, P(\bold{x}_{t} \geq 1 \text{ and } \bold{x}_{t+m} \geq 1))
\end{equation*}

where

\begin{equation*}
P(\bold{x}_{t} \geq 2) = 1 - \binom{n}{0} \zeta^0 (1-\zeta)^n - \binom{n}{1} \zeta^1 (1-\zeta)^{n-1}
\end{equation*}

The probability that both elements in the sequence are one or greater:

\begin{equation*}
P(\bold{x}_{t} \geq 1 \text{ and } \bold{x}_{t+m} \geq 1) = (1 - (1-\zeta)^n)^2
\end{equation*}
