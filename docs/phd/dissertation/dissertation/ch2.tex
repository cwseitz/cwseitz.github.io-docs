...
\subsection{Application of deep models in microscopy}
...
In the remainder of this paper, we introduce KD estimation as an alternative to direct localization using low-resolution images, followed by demonstration of our variational diffusion model for measuring uncertainty KD estimation at scale. 
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[thick,scale=1.5, every node/.style={scale=1.5}]
        ...
        \node[rectangle, draw=none, fill=white, scale=0.66, right=2.25pt of x, yshift=9pt] (eq1) {$\mu_{1}$};
        \node[rectangle, draw=none, fill=white, scale=0.66, right=2.25pt of z1, yshift=9pt] (eq2) {$\mu_{2}$};
        \node[rectangle, draw=none, fill=white, scale=0.66, below=-0.27pt of z3] (eq3) {$\mu_{t}$};
    ...
    \end{tikzpicture}
    \caption{\textbf{Diffusion model with Markovian transitions}}
    \label{fig: mt}
