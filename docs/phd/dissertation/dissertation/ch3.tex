...
\begin{equation}
\sigma_{g^{(2)}(0)}=\sqrt{\frac{p(1-p)}{N_{\mathrm{frames}}q^{2}}}
\end{equation}

The value of $\sigma_{g^{(2)}(0)}$ is therefore a function of $\zeta$ as well as the number of frames in the acquisition $N_{\mathrm{frames}}$. 
