...
\setbeamertemplate{footline}{
    ...
    \begin{beamercolorbox}[wd=\paperwidth,ht=1.0ex,dp=1.5ex,leftskip=2ex,rightskip=2ex]{page ooter}%
        \footnotesize
        \usebeamerfont{title in head/foot}%
        Probabilistic modeling approaches to fluorescence nanoscopy \hfill
            %\insertsection \hfill
        \insertframenumber{} / \inserttotalframenumber
    \end{beamercolorbox}}%
}
...
\item Likelihood on counts: $\mathcal{L}(n_t\lvert N,\zeta) = \mathrm{Poisson}(\lambda)* \mathrm{Binomial}(N,\zeta)$
\end{itemize}
\end{textblock*}
...
\end{textblock*}
\begin{textblock*}{14cm}(1.0cm,8.0cm)
...
\setbeamertemplate{footline}{
    ...
    \begin{beamercolorbox}[wd=\paperwidth,ht=1.0ex,dp=2ex,leftskip=2ex,rightskip=2ex]{page footer}%
        \footnotesize
        \usebeamerfont{title in head/foot}%
        \textbf{Seitz C.} et al. 2024 (Submitted) https://doi.org/10.1101/2024.09.03.611057\hfill
        \insertframenumber{} / \inserttotalframenumber
    \end{beamercolorbox}}%
}
...
\item \textbf{Clayton Seitz}, Donghong Fu, Mengyuan Liu, Hailan Ma, and Jing Liu. \textit{BRD4 phosphorylation regulates the structure of chromatin nanodomains}. Submitted. https://doi.org/10.1101/2024.09.03.611057. 2024
