...
\subsection{Approach 2: Integrate single photon counting and widefield single molecule imaging}

\begin{frame}{An integrated single photon avalanche diode (SPAD) array}
\begin{itemize}
\item SPAD arrays open several doors in microscopy, including new modeling approaches
\end{itemize}
...
\item Can be thought of as a type of Hidden Markov Model (HMM) 
\end{itemize}
\end{textblock*}
...
\setbeamertemplate{footline}{
    ...
    \begin{beamercolorbox}[wd=\paperwidth,ht=1.0ex,dp=2ex,leftskip=2ex,rightskip=2ex]{page footer}%
        \footnotesize
        \usebeamerfont{title in head/foot}%
        \textbf{Seitz C.} et al. 2024 (Submitted)\hfill
        \insertframenumber{} / \inserttotalframenumber
    \end{beamercolorbox}}%
}
