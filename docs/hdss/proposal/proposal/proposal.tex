...
\begin{equation*}
P_{\phi}(\mathbf{x}) = \frac{P_{\phi}(\mathbf{x|z})P_{\phi}(\mathbf{z})}{P_{\psi}(\mathbf{z|x})}
\end{equation*}
...
\begin{align*}
\log P_{\phi}(\mathbf{x}) &= \mathbb{E}_{\mathbf{z}\sim P_{\phi}(\mathbf{z|x})} \log\frac{P_{\phi}(\mathbf{z})P_{\phi}(\mathbf{x|z})}{P_{\phi}(\mathbf{z|x})}
\end{align*}
