...
\\
\\
Assuming that $\text{im}(H) = \mathbb{F}_{2}^{m}$, we need to capture this ``lack of bijectivity" of our compression scheme. To do this, we consider the probability that our $\text{Decom}$ decompresses $w \in \mathbb{F}_{2}^{m}$ to a draw from the input distribution. Therefore,
...
has distance at least $t/\log(1/p)$ if $H$ is the compression matrix defined above. To see this, we will try to find $z\in \mathbb{F}_{q}^{n}$ such that $\text{Decom}(Hz)$ is correct 
