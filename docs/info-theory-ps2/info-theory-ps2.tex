...
where $T(P)$ denotes the type of an empirical distribution $P$. For the lower bound, the arbitrary type $T(P)$ becomes any $\bar{x}\sim Q^{n}$ such that the number of heads in $\bar{x}$ is at least $n(1-\epsilon)$. 

\begin{equation*}
\underset{\bar{x} \sim Q^{n}}{\mathbf{Pr}}\left[\sum_{i}\bar{x}_{i} \geq 
n(1-\epsilon)\right]\geq 
(n+1) \cdot 2^{-nD(P||Q)}
\end{equation*}

At this point we substitute a desirable error rate of 1/10 for our classifier and take the logarithm of both sides to get 

\begin{align*}
(n+1) \cdot 2^{-nD(P||Q)} &\leq 1 + n(1 - D(P||Q))\\
&= 1 + n(1 - \frac{1}{2\ln 2}\cdot \frac{\epsilon^{2}}{1-2\epsilon})\\
&\leq -\log 10
\end{align*}

So, we can conclude for the upper bound that

\begin{align*}
n \leq \frac{1-2\epsilon}{\epsilon^{2}}
\end{align*}

Now for the upper bound the prefactor cancels and we have

\begin{equation*}
\underset{\bar{x} \sim Q^{n}}{\mathbf{Pr}}\left[\sum_{i}\bar{x}_{i} \geq 
n(1-\epsilon)\right] \leq 2^{-nD(P||Q)}
\end{equation*}
