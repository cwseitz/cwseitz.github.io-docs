...
As for the upper bound on $n$, we can use Chernoff bounds. Chernoff bounds tell us the probability that a sequence drawn from the product distribution $Q^{n}$ deviates from $\mathbb{E}_{\bar{x}\sim Q^{n}}[\bar{x}]$ where $\bar{x} = (X_{1}\; ... \; X_{n})$. We seek the probability that $\bar{x}$ deviates from the expectation by at least $2\epsilon$. For a bias of $1-2\epsilon$ we expect a sum over $\bar{x}$ to give $n(1-2\epsilon)$. So if we draw $\bar{x}$ from $Q^{n}$, we expect that

\begin{equation*}
\underset{Q^{n}}{\mathbb{P}}\left[ \sum_{i}X_{i} \geq n(1-2\epsilon) \right] = (n+1)\cdot 2^{-n\cdot D(P||Q)} \leq (n+1)\cdot 2^{-\frac{n\epsilon^{2}}{2\ln 2(1-2\epsilon)}}
\end{equation*}

If we require that $9/10$ of our predictions are correct, then

\begin{equation*}
(n+1)\cdot 2^{-\frac{n\epsilon^{2}}{2\ln 2(1-2\epsilon)}} \geq \frac{1}{10}
\end{equation*}
