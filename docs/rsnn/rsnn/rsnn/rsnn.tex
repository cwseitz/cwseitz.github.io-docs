...
\subsection{Fokker-Planck Solution for Unbounded Diffusion}
 ...
 An important difference in the IF model in (1.1) and the OU process is that the voltage variable $V$ in the IF model is bounded below by $V_{R}$ and above by the threshold $\Theta$ while the OU process is not necessarily bounded. For the unbounded case of the OU process, the transition density $T(V',t'|V,t)$ has a known analytical form:
...
\subsection{The Schr\"{o}dinger Renewal Ansatz}
