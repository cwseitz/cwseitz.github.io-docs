...
The stationary solution of the density $P(V,t)$ has been found for simple models such as when $\psi(V) = -V$, which resembles the Ornstein-Uhlenbeck process with special boundary conditions imposed by the threshold (Brunel 2000; Brunel and Hakim 1999). However, the solution for non-linear integrate and fire models frequently requires numerical integration of (1.5) to find the stationary density over the membrane potential and to estimate steady state firing rates. A novel approach, based on linear response theory, leverages the diffusion approximation to make predictions on spike train correlations directly in terms of circuit architecture (Ocker 2017).

In essence, a linear response theory of network dynamics determines the auto- and cross-correlations of spike trains by defining a linear interaction between presynaptic and postsynaptic firing rates. For example, the approach given by Pernice et al. leverages Hawkes theory of linearly interacting point processes to calculate stationary firing rates and spike correlations in excitatory-inhibitory networks with arbitrary topology (Pernice 2011, 2012). Other studies have used a similar approach in which the linear response function of a neuron is found by numerically integrating the corresponding Fokker-Planck equation with a weak sinusoidal perturbation to the synaptic current (Richardson 2007; 2009). In the following paragraphs we will summarize the latter approach as it has been adapted for more realistic neuron models such as the EIF.

The first step is to compute the stationary density for the membrane potential by solving the Fokker-Planck equation under white-noise stimulation. This simultaneously provides the steady state firing rate by evaluating the density at threshold $P(\theta,t)$. The following is a brief summary of the procedure for finding the stationary density, see (Richardson 2007) for a full derivation. Now, (1.5) is often reformulated as the continuity equation
...
Then, this solution is used to find the first passage time density from which we can extract the spike train power spectrum. After additionally calculating the linear response of a neuron to input and the synaptic kernel in the frequency domain, we can solve the following set of matrix equations for the cross-spectra (cross-covariance) in the network


The quantity $J$ is called the probability current. Details of the numerical integration of (1.5) were given in (Richardson 2007). This result will be discussed later in the context of a linear response theory. 
