...
\begin{align*}
k_{ij}(\mathbf{r}_{i}, \mathbf{r}_{j}) = \frac{1}{2\pi\sqrt{|\Sigma_{i}|}}\exp\left(-\frac{1}{2}(\mathbf{r}_{i} - \mathbf{r}_{j})^{T}\Sigma_{i}^{-1}(\mathbf{r}_{i} - \mathbf{r}_{j})\right)
\end{align*}

where $\Sigma_{i}$ is a two-dimensional covariance matrix. We assume that connectivity is radially symmetric and therefore $\Sigma_{i} = \sigma_{i}^{2}I$ where $I$ is the identity matrix. The property $k_{ij} \neq k_{ji}$ has important consequences. In the case that $k_{ij} = k_{ji}$ the direction $i\rightarrow j$ and $j\rightarrow i$ have equal probability. However, when $k_{ij} \neq k_{ji}$, we need the follow normalization when determining direction of the synapse
