...
which, with a simple change of variables, becomes a leaky integrate and fire neuron model 

\begin{equation*}
\dot{V}(t) = -\frac{V(t)}{\tau} + \mu(t) + \sigma\sqrt{dt}\eta(t)
\end{equation*}

we define $I(t) = \mu(t) + \sigma\sqrt{dt}\eta(t)$ as the input current arriving at the soma of a neuron and $\eta(t) \sim \mathcal{N}(0,1)$. To illustrate the applicability of (2.10) in predicting the distribution $P(V,t)$, we consider the case where $\eta(t)$ is a 1D Gaussian white noise. We have the following Fokker-Planck equation
...
\subsection{Numerical solution to the 1D FP equation}

We will now show that (2.8) can be integrated using finite central differences method which gives a stable solution

\begin{align*}
P(V_{j}, t+\Delta t) &= P(V_{j},t) - \Delta t\left[\alpha \cdot \frac{P(V_{j+1}, t) - P(V_{j},t)}{\Delta V}\right] \\
&+ \Delta t\left[\frac{\sigma^{2}}{2}\frac{P(V_{j+1}, t) - 2P(V_{j},t) + P(V_{j-1},t) }{\Delta V^{2}}\right]
\end{align*} 
