...
We now proceed to define the adjacency matrix $\mathcal{C} \in \mathbb{F}_{2}^{N\times N}$ where $\mathcal{C}_{ij} = 1$ defines a synapse from $i\rightarrow j$. The matrix $\mathcal{C}$ defines a directed graph which suggests that we need a mechanism of defining the probabilities $p_{i\rightarrow j}, p_{j\rightarrow i}, q$ where $q$ is the probability of no synapse. In the case that $k_{ij} = k_{ji}$ then $p_{i\rightarrow j} =  p_{j\rightarrow i}$. However, when $k_{ij} \neq k_{ji}$, we need to carefully define these three probabilities. This is possible if we interpret $p_{i\rightarrow j}$ and $p_{j\rightarrow i}$ as Bernoulli random variables $X$ and $Y$. For example $X = 1$ when there is a synapse $i\rightarrow j$ and $Y = 1$ when there is a synapse $j\rightarrow i$. We have the following joint probabilities
  ...
  \[
    \mathcal{P} = \left\{\begin{array}{lr}
        p_{i\rightarrow j} = p(X=1|Y=0)\cdot p(Y=0) = \frac{k_{ij}(1-k_{ji})}{Z}\\
        p_{j\rightarrow i} = p(X=0|Y=1)\cdot p(Y=1) = \frac{k_{ji}(1-k_{ij})}{Z}\\
        q = p(X=0|Y=0)\cdot p(Y=0) = \frac{(1-k_{ij})(1-k_{ji})}{Z}\\
        p_{i\leftrightarrow j} = p(X=1|Y=1)\cdot p(Y=1) = 0\\
        \end{array}\right\}
  \]

with $Z = k_{ij}(1-k_{ji}) + k_{ji}(1-k_{ij}) + (1-k_{ij})(1-k_{ji})$. Notice that the last equality is true by the definition $p(X=1|Y=1) = 0$. The first two moments of  the number of incoming and outgoing connections (in-degree and out-degree) then are given by binomial statistics. The first two moments of the in-degree are given by

\begin{align*}
\langle N_{ij} \rangle = \frac{1}{Z}\sum_{j} k_{ij}(1-k_{ji}),\;\;\;\;\;\;\;\; \mathrm{Var}(N_{ij}) = \frac{1}{Z}\sum_{j} k_{ij}(1-k_{ji})(Z-k_{ij}(1-k_{ji})
\end{align*}

and the first two moments of the out-degree

\begin{align*}
\langle N_{ji} \rangle = \frac{1}{Z}\sum_{i} k_{ji}(1-k_{ij}) ,\;\;\;\;\;\;\;\; \mathrm{Var}(N_{ji}) = \frac{1}{Z}\sum_{i} k_{ji}(1-k_{ij})(Z-k_{ji}(1-k_{ij})
\end{align*}


A connectivity matrix can be generated by iterating over the upper triangle of $\mathcal{C}$, each iteration sampling $x\sim U([0,1])$ and applying the conditional

  \[
    \left\{\begin{array}{lr}
        i\rightarrow j = 0 \leq x \leq \frac{k_{ij}(1-k_{ji})}{Z}\\
        j\rightarrow i = \frac{k_{ij}(1-k_{ji})}{Z} \leq x \leq \frac{k_{ji}(1-k_{ij})}{Z}\\
        \end{array}\right\}
  \]
...
\begin{align*}
\langle N_{ij} \rangle &= \frac{1}{Z_{ij}}\sum_{j} k_{ij}(1-k_{ji})\\
&= \frac{1}{Z_{ij}}\left(\sum_{x,y} \frac{\rho_{i}}{\sigma_{i}\sqrt{2\pi}}\exp\left(-\frac{1}{2}\frac{|\mathbf{r}-\mathbf{r}_{0}|^{2}}{\sigma_{i}^{2}} \right) - \frac{\rho_{i}^{2}}{2\pi\sigma_{i}^{2}}\exp\left(-\frac{|\mathbf{r}-\mathbf{r}_{0}|^{2}}{\sigma_{i}^{2}} \right)\right)\\
\end{align*}

Rather than approximate the above summations with gaussian integrals, we will compute $\langle N_{ij}\rangle$ numerically. The normalization constant is given by 

\begin{align*}
Z_{ij} &= k_{ij}(1-k_{ji}) + k_{ji}(1-k_{ij}) + (1-k_{ij})(1-k_{ji})\\
&= 1 + k_{ij}^{2}\\
&= 1 + \frac{1}{2\pi}\left(\frac{\rho_{i}}{\sigma_{i}}\right)^{2}\exp\left(-\frac{|\Delta\mathbf{r}_{ij}|^{2}}{\sigma_{i}^{2}} \right)
\end{align*}

\subsection{Spatial Correlations on a 2D Gaussian Lattice}

We can now examine the probability of two neurons sharing a number of connections as a function of their distance $|\mathbf{r}_{ij}|$. This problem reduces to the The probability neuron $i$ and neuron $j$ share an incoming connection from neuron $w$ reads

\begin{align*}
\langle S_{ij} \rangle = \frac{1}{Z^{2}}\sum_{w} k_{wi}(1-k_{wi})k_{wj}(1-k_{wj})
\end{align*}
