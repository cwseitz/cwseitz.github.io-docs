...
We begin the theoretical discussion of spiking dynamics by adopting the terminology for network architectures introduced in (Renart 2010). We say a network is \emph{sparsely} connected if the average in-degree of a neuron scales as $\mathcal{O}(1/N)$ meaning it is independent of the network size. Naturally, we then say that a network is \emph{dense} if the in-degree scales as $\mathcal{O}(1)$. At the same time, a network is said to have \emph{strong} coupling if the fraction of inputs that need to be active for the cell to fire is small. Thus, for dense networks the synaptic coupling must scale at least as fast as $\mathcal{O}(1/N)$. In previous work, the synaptic couplings have been taken to scale as $\mathcal{O}(1/\sqrt{N})$ (Renart 2010, Rosenbaum 2017). This scaling has been chosen s.t. the fluctuations in the synaptic input currents to the neurons in an asynchronous network saturates to a value of the order of the spiking threshold in the limit of large networks (Vreeswijk, 1996).
...
\begin{align}
\langle \zeta_{\alpha}, \zeta_{\beta}\rangle &= \langle \mathrm{CC}(F_{j}^{\alpha} + R_{j}^{\alpha},F_{k}^{\beta} + R_{k}^{\beta})\rangle_{j,k}\\
&= \langle \mathrm{CC}(F_{j}^{\alpha},F_{k}^{\beta}) + \mathrm{CC}(F_{j}^{\alpha},R_{k}^{\beta})\\
&+ \mathrm{CC}(R_{j}^{\alpha},F_{k}^{\beta}) + \mathrm{CC}(R_{j}^{\alpha},R_{k}^{\beta})\rangle_{j,k}
\end{align}

\begin{align*}
\mathrm{CC}(R_{j}^{\alpha},R_{k}^{\beta}) &=\mathrm{CC} \left(\left(\sum_{i\neq j}J_{ij}Z_{i}(t)\right)\left(\sum_{i\neq k}J_{ik}Z_{ik}(t+\tau)\right)\right)\\
&= \mathrm{CC} \left(\left(J_{jk}Z_{k}(t) + \sum_{i\neq j,k}J_{ij}Z_{ij}(t)\right)\left(J_{kj}Z_{j}(t+\tau) + \sum_{i\neq j,k}J_{ik}Z_{ik}(t+\tau)\right)\right)\\
&= \mathrm{CC} \left(\left(J_{jk}Z_{k}(t) + \sum_{i\neq j,k}J_{ij}Z_{ij}(t)\right)\left(\sum_{i\neq j,k}J_{ik}Z_{ik}(t+\tau)\right)\right)\\
&= \mathrm{CC} \left(J_{jk}Z_{k}(t),\mu_{k}\right) + \mathrm{CC} \left(\mu_{j},\mu_{k}\right)
\end{align*}
